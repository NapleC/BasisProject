...
/**
 * 类名: SpanUtil
 * 知识点来自 CSDN 博客：http://blog.csdn.net/u012422440/article/details/52133037
 *
 *
 * Description: 使 TextView 显示不同（颜色、大小）文字的工具类
 * 使用的关键类：
 * SpannableStringBuilder：使用 setSpan(span, start, end, flag); 来设置文字的 Span 格式
 ...
 *
 *
 * Android Span 框架涉及的类遵循一下四个定义规则:(ctrl + H 查看层级结构（Android Studio），上面 XxxSpan 是这四个类或接口的部分子类或实现类)
 ...
 * 虽然有以上四个规则，但是我们自定义 Span 时并不需要直接继承或者实现这些类或者接口，而是实现其他更加具体的类。
 *
 *
 *
 *
 ...
 *    .removeSpans("这是", 2) // 移除下标 2 后第一个 "这是" 的 Span 样式
 *    .setForeColor("字体", Color.MAGENTA) // 渲染最后一个 "字体" 的字体颜色为 Color.MAGENTA
 *    .clearSpans() // 清除所有格式
 ...
 *    .showIn(mTvPackage); // 显示到控件 TextView 中
 *
 *  SpanUtil.create()
