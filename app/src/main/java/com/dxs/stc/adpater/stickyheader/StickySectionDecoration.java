...
public class StickySectionDecoration extends RecyclerView.ItemDecoration {
    ...
    public StickySectionDecoration(Context context, GroupInfoCallback callback) {
        ...
        mTextSize = context.getResources().getDimensionPixelOffset(R.dimen.sp_10);

        mHeaderHeight = (int) Math.max(mHeaderHeight, mTextSize);

        mTextPaint = new TextPaint();
        mTextPaint.setColor(ContextCompat.getColor(context, R.color.color_69));
...
//        mTextPaint.setTextAlign(Paint.Align.CENTER);
        ...
        mPaint.setColor(ContextCompat.getColor(context, R.color.color_E6));
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mCallback != null) {
            ...
            // 如果是组内的第一个则将间距撑开为一个 Header 的高度，或者就是普通的分割线高度
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mCallback != null) {
                ...
                if (i != 0) {
                    ...
                    // 当 ItemView 是屏幕上第一个可见的 View 时，不管它是不是组内第一个 View
    ...
    private void drawHeaderRect(Canvas c, GroupInfo groupinfo, int left, int top, int right, int bottom) {
        ...
        float titleY = bottom + mFontMetrics.ascent;
        // 绘制 Title
